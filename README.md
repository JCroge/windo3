...
- 默认上线状态：`TACTICAL_TRACK_ENABLED=false` 且 `TACTICAL_SHADOW_ONLY=true`。也就是生产默认不改变 live 开仓；启用必须先经过 replay/shadow 证明，再小额灰度。
...
- 出口轨道：Main Trend Runner 与 Tactical Exit Track 分离，Tactical 默认 shadow-only，字段经 Reviewer/Counterfactual 分桶
...
| Tactical 出口轨道 | `TACTICAL_TRACK_ENABLED` / `TACTICAL_SHADOW_ONLY` / `TACTICAL_MAX_LEVERAGE` / `TACTICAL_TP1_R` / `TACTICAL_MAX_HOLD_MINUTES` | 默认 disabled + shadow-only；开启后弱/混合候选使用独立 Tactical R:R/EV 与落袋退出 |
