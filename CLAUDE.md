...
- 当前系统是多 Agent 加密货币趋势交易系统，不是跨交易所套利系统；生产、paper、testnet 和实盘验收主入口统一为 `python3 run_agents.py`。
- `main.py` 和 `live_trading.py` 是归档/调试路径，不能作为生产入口。
- 云服当前为 Tactical V2 `LIVE`，固定 `100U x 3`，Sidecar 只 resident monitoring 且 `admission_enabled=false`。代码默认 `TACTICAL_V2_MODE=off`，线上事实必须以 `.env`、启动 banner 和 `data/tactical_v2_status.json` 为准。
- V2 持仓 owner 为 `strategy_owner=tactical_v2`，Main 不得改动其 TP/SL、trailing、thesis 或 Position Analyst 决策；全局安全路径仍可 owner-bound 全平。
- 2026-08-05/06 的入口精确回查、保护 halt 自愈、旧 halt 迁移和 durable final-PnL replay 已部署；所有 recovery 仍以交易所、owner、订单、仓位、数量和保护证据为准，不能靠删除状态文件或重启绕过。
- Shadow Tactical sidecar 是独立进程，入口 `scripts/shadow_tactical_live_sidecar.py`；Main 不得 backfill sidecar-owned 仓位，`net_mode` 同标的堆叠和 ghost exposure 必须 fail-closed。
- 当前 Go/No-Go：V2 首轮固定 cohort 继续观察；禁止擅自恢复 sidecar admission 或扩大 V2 槽位/保证金。云服没有已部署的应用级 supervisor，重启使用 `/restart` 或受控 OS 进程流程。
...
- Main/Tactical 出口语义必须通过 `track`、`exit_profile`、`slot_type`、`tactical_close_reason` 透传到 plan、attribution、position、execution_result、Reviewer；不要用 `regime + side` 反推。
