...
- 当前基线：`1238 passed / 4 deselected / 1 warning`（2026-06-15；1223 之上叠加 `decision-tape-capture-fix` +11、`tick-capture-retention-prune` +4，均 observability-only，comet 全流程归档入 main）。1223 = 1149 之上叠加**反事实策略实验室 L1-L4**（5 个 comet change：`counterfactual-replay-foundation` / `deterministic-replay-golden-master` / `perturbation-replay-per-decision` / `sequential-portfolio-perturbation` / `perturbation-knob-sweep`，observability-only）。1149 由第五次审计 + ccxt keysort 修复 + Agent 故障可见性 + 持仓同步重试 + Agent Health Supervisor + tick-loop 挂死检测 + bot LLM env 隔离构成。
