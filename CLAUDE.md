...
- 2026-06-11 第五次审计 P1-01/P2-02 + 第二梯队 6 项 robustness 修复（comet changes `add-position-tp-sink-halt-recovery` + `fail-closed-robustness-hardening`，叠在同一分支）。**P1-01**：`executor.py:add_to_position` 加仓后只写 scalar `take_profit` 不更新 `take_profit_levels`/不经 `_set_position_tp` → 下轮 `_update_trailing` 顶部 TP 不变量守卫命中 → `_halt_symbol('tp_invariant_breach')` 无条件跳全局熔断 → 全系统停止开仓且不自愈。修：按每个 level 距旧均价比例平移整个 `take_profit_levels` 并经 `_set_position_tp` 单点收口（tp_filled-safe）。**P2-02**：`/resume_symbol` 清 per-symbol halt 后若全局 halt 仍在，`symbol_halt_cleared` 附 `global_halt_active`，TG 诚实回显"全局仍 halt，请用 /resume"。**P2-03**：`_handle_resume` 非 matched 对账结果一律 fail-closed 维持熔断（删对 PnL `Reconciler` 不存在的 `reconcile` 死调用 + 收紧无 reconciler 的 else fail-open）；绕过对账只能 `/force_resume`。**P2-06**：`_handle_risk_alert` 顶部 `source=='paper_executor'` 结构性守卫（paper 永不驱动 live 平仓）。**P2-16**：`orchestrator._maybe_alert_dlq_growth` DLQ 增长主动 `telegram_alert{bus_dlq_growth}`。**P2-17**：`config_loader.clamp_to_hard_limits` + executor config 兜底复用（杜绝风险限额 fail-open）。**P2-20**：`event_journal` write 后 `os.fsync`。**P2-21**：`halt_state._save` 删非原子裸写兜底改 logger。两 change comet 全流程归档；两条线（本批 + P1-02/P1-03）合并入 main 后**全量实测 `1088 passed / 4 deselected / 1 warning`**。另修复 comet-archive 的 delta→master 同步 bug（原 `cp` 盲覆盖丢需求，改为 ADDED 追加/MODIFIED 替换/REMOVED 删除），并重建被覆盖的 4 个 master spec。详见 `docs/generated_reports/系统性审计报告_20260610_第五次.md` 与 `docs/superpowers/specs/2026-06-11-*-design.md`。
- 当前待办统一看 `docs/to-do-list.md`，最新审计报告看 `docs/generated_reports/系统性审计报告_20260610_第五次.md`。
