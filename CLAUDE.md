...
- 当前基线（2026-06-24 全量实测）：`1430 passed / 0 failed / 4 deselected`（+14 = `cf-choppy-neutral-tp1-floor-ab` 驱动 13 测试 + 红线守卫 1，observability-only，已归档入 main）。历史基线 `1416`（2026-06-23 全套件首次全绿）含本轮形态研究链新增测试（`test_candlestick_patterns.py` +19、`test_pattern_forward_shadow.py` +5、红线守卫 +1）；预存的 `test_decision_replay.py::test_no_unclassified_missing_snapshot_keys` fail 已由 `fix-replay-register-reversal-pseudo-keys` 收掉；06-21 记录的 8 个 asyncio event-loop 全量污染 fail（`test_round2_probe_long_dispatcher`/`test_round2_request_id_position`，隔离单跑全 PASS、非 change 引入）本次全量运行未复现。历史基线 `1359`（2026-06-21）、`1338`（2026-06-20）、`1314`（2026-06-18）、`1302`（2026-06-17）。
- **2026-06-21 归 1 个 comet change（基线 1338→1359，新 capability `regime-aware-long-entry-guard`，改 live 开仓决策，已合并 main 但**未推送、需重启 live 加载**）——多头过热阈值体制感知**：针对策略诊断（现策略方向无 edge、信号分↔方向相关性≈0、问题在入场侧）的病根「choppy 里追突破」，让 `_check_entry_position_policy` 的 overheat `range_pos` 阈值经 `_resolve_long_range_thresholds(eff_regime)` 按体制取——`choppy/mixed/bearish` 收紧（生产起步 0.70/代码目标 0.55）转 `deferred_pullback_overheat`，`bullish` 保 0.82，总开关 `long_live_regime_aware_range_enabled` 可即时回退。归因加 `entry_regime_used`/`entry_range_pos_threshold` + policy `long_overheat_v2_regime`（部署后按体制切分核对 choppy 多单 PF）。详见上方硬约束条目 + `docs/superpowers/reports/2026-06-21-regime-aware-long-entry-guard-verify.md`。诊断全貌见 memory `strategy-no-directional-edge-diagnosis`（四病根，本 change 只做病根2+4）。
...
- **体制感知阈值（2026-06-21 `regime-aware-long-entry-guard`，新 capability，已合并 main）**：overheat 的 `range_pos`/`daily_gain_range_pos` 阈值不再固定，经 `Judge._resolve_long_range_thresholds(eff_regime)` 单一收口按体制取——`choppy/mixed/bearish` 收紧（config `long_live_max_range_pos_choppy`/`long_live_daily_gain_range_pos_choppy`，代码默认 0.55/0.50，**生产 config.yaml 缓进起步 0.70**），`bullish`/None/未知/总开关 `long_live_regime_aware_range_enabled=false` → 默认 0.82/0.75。体制取值用 `self._regime_manager.snapshot()['effective_regime']`（与 `_apply_regime_policy` 同源，不传参）。归因加 `entry_regime_used`/`entry_range_pos_threshold`（放行经 plan→`_build_attribution`，overheat 经内联 attr）+ policy 标记 `long_overheat_v2_regime`。**两处已知分歧待后续 change 收口**：① `event_backtest.py` 的 overheat 仍固定 0.82、未体制感知（backtest 无组合级 regime）；② `_rejection_attribution`(3186) 函数体未加这两个新字段（仅 `_build_attribution` 与 overheat 内联路径有），非 overheat 拒单不带该字段。详见 `docs/superpowers/specs/2026-06-21-regime-aware-long-entry-guard-design.md` + memory `regime-aware-long-entry-guard-change`。
...
- **形态研究链 observability-only（2026-06-23，changes `daily-pattern-edge-lab`/`fix-fetch-subdaily-backward-pagination`/`pattern-forward-shadow-recorder`，新 capability `pattern-edge-discovery`+`pattern-forward-shadow`）**：日线蜡烛形态 edge 发现与前向影子验证全链——`utils/candlestick_patterns.py`（~28 形态库，固定阈值禁调）/ `cf_pattern_edge_discovery.py`（回测：ATR 退出 1.5×/3.0×/10日 + 上下文6桶[range_pos 20日3档×MA50趋势] + OOS 三分[train2023-24/val2025/test2026] + Bonferroni/FDR + `cf_honesty_gate`，`--interval` 窗口 interval 感知）/ `pattern_forward_shadow.py`（`--record`/`--settle` 前向 record-only）/ 历史数据 `fetch_historical_klines.py`→`data/klines.db`（分页向历史纵深，日线 2.75年/4h ~1.85年）——全部 **observability-only**：**严禁**任何决策/风控路径（judge/executor/portfolio_risk_guard/reviewer/position_analyst）import 这些模块，守卫 `tests/test_cf_red_line_guard.py::test_decision_paths_do_not_read_pattern_research`（forbidden 集 `candlestick_patterns`/`cf_pattern_edge_discovery`/`pattern_forward_shadow`）。复用 `resolve_counterfactual`+`cf_honesty_gate`。**已确认 1 信号**：`Bearish Engulfing | 低 range_pos | 跌势`（日线 +0.326R / 4h 时间对齐 +0.208R，跨周期确认；Evening Star 候选 4h 翻负否决），**进入前向影子验证**（每日 cron `--record` + 定期 `--settle`，见 README；须数周成熟，诚实门拒答薄样本）。**确认稳健前不上实盘、不改 config。** 详见 memory `daily-pattern-lab-candidates`/`alpha-source-hunt-verdict`（根结点=赌动量但市场无动量、全 alpha 源证伪）+ `docs/superpowers/specs/2026-06-23-*-design.md`。
- **choppy+neutral TP1 地板反事实驱动 observability-only（2026-06-24，change `cf-choppy-neutral-tp1-floor-ab`，新 capability，新驱动 `cf_choppy_neutral_tp1_floor_ab.py` 镜像 `cf_ev_decouple_ab.py`）**：量化「choppy+neutral 多单要求 TP1 口径 `effective_rr_tp1` ≥ 地板（而非阶梯 effRR）」的反事实 PnL delta。对磁带 accept 流 **ladder-toggle 两臂复盘**（`replay(ladder_rr_enabled=True)`=baseline 自检 vs `False`=CF TP1 地板，机制：`_build_plan`→`_effective_rr_for_plan` 在 ladder off 返回 TP1 口径，floor gate 比之）+ baseline 复现自检闸 + **翻转纯度**（只计 `rr_below_floor` 翻转→`tp1_floor_rejected`，非地板翻转→`other_flip` 不结算）+ `resolve_counterfactual`/klines TP1 保守结算（`_plan` 传 `entry_price`/`created_at` 非 `entry_ref`）+ `cf_honesty_gate(min_sample=30 不下调)`。主桶 choppy+neutral、旁路 mixed+neutral。**严禁**任何决策/风控路径（judge/executor/portfolio_risk_guard/reviewer/position_analyst）import 本驱动，守卫 `tests/test_cf_red_line_guard.py::test_decision_paths_do_not_read_choppy_tp1_floor_ab`。**真跑结论**：主桶 195 accept→86 忠实/109 失真排除，84/86(98%)TP1≥地板翻 reject（全 rr_below_floor）、避开桶 −0.50R/簇（1tp/12sl=8%胜率）、mixed 旁路 0 accept；诚实门 INSUFFICIENT_SAMPLE(n=13)→**方向强但 suggestive，不改 config/不上 live**。背景：边缘60亏损单深查 13/13 全 choppy+neutral+effRR 贴 1.50 地板/真实 TP1 口径全<1.50，lever2 阶梯+ev-decouple 联合放行。**周更 cron 已装并验证通过**（`crontab` 周一 10:13 重跑驱动→`~/Library/Logs/cf_choppy_tp1_floor_ab.log`，等 n≥30 诚实门跨过；`/usr/sbin/cron` 的 Full Disk Access **2026-06-24 已授权 + `sudo killall cron` 重起 + 11:19 cron 实跑通过**——repo 在 ~/Desktop 是 TCC 保护目录，FDA 失效/换机会报 Operation not permitted，重授后须 `sudo killall cron`，见 README §周更 cron）。详见 memory `cf_lab_strategy_diagnosis_winrate` + `docs/superpowers/specs/2026-06-24-cf-choppy-neutral-tp1-floor-ab-design.md`。
